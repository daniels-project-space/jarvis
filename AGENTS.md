...
- **Live lane** — turn-taking browser audio uses free Whisper transcription, the same subscription-backed Codex app-server as text, and worker-isolated local KittenTTS (Jasper). Capture is gated while Jarvis speaks, preventing self-echo cut-offs without repeatedly reopening the physical microphone. Tools execute via `/api/tools`; finished turns live in Convex history.
