...
function digestStamp(value: string | null) {
...
function WorkerDetail({ node, onBack }: { node: FleetNode; onBack: () => void }) {
  ...
  return <section data-fleet-worker-detail className="flex min-h-0 flex-1 flex-col gap-2 rounded-xl border border-white/[0.08] bg-black/20 p-3">
    ...
    <div className="grid grid-cols-2 gap-1 text-[9px] text-slate"><div className="truncate">stage · <span className="text-ice">{node.stage}</span></div><div className="truncate">merge · <span className="text-ice">{node.mergeState}</span></div><div className="truncate">handoffs · <span className="text-ice">{node.dependenciesReady}/{node.dependencyCount}</span></div><div className="truncate">runtime · <span className="text-ice">{node.workerRuntime ?? "not assigned"}</span></div><div className="col-span-2 truncate">receipt · <span className="font-mono text-ice">{node.receipt.state}{node.receipt.integrationDigest ? ` · integration ${digestStamp(node.receipt.integrationDigest)}` : node.receipt.reviewDigest ? ` · review ${digestStamp(node.receipt.reviewDigest)}` : ""}</span></div>{node.decision.kind !== "none" && <div className="col-span-2 truncate">decision · <span className="text-amber">{node.decision.kind} · {node.decision.detail}</span></div>}{(node.stall.count > 0 || node.stall.at || node.stall.reason) && <div className="col-span-2 truncate">stalls · <span className="text-amber">{node.stall.count} · {node.stall.reason ?? progressStamp(node.stall.at)}</span></div>}<div className="col-span-2 truncate">last meaningful progress · <span className="font-mono text-ice">{progressStamp(node.progressAt)}</span></div></div>
    ...
    <aside data-fleet-surface="expanded" aria-label="Live Jarvis fleet" className="materialize glass absolute inset-x-1 bottom-1 z-40 flex h-[min(78vh,680px)] min-h-0 flex-col overflow-hidden rounded-2xl !border-cyan/25 bg-[#071019]/96 p-3 shadow-2xl md:inset-y-2 md:left-2 md:right-auto md:h-auto md:w-[min(760px,62%)]">
      ...
      <div className="mt-2 flex min-h-0 flex-1 gap-2 overflow-hidden">
        {selectedId ? (selected ? <WorkerDetail node={selected} onBack={() => selectJob(null)} /> : <div data-fleet-detail-loading className="flex flex-1 items-center justify-center text-xs text-cyan">loading exact work detail…</div>) : <div className="scrollbar-thin min-h-0 flex-1 space-y-2 overflow-auto pr-0.5">
          {(attention.length > 0 || fleet.truthWarnings.length > 0) && <section aria-labelledby="needs-daniel-heading" className="rounded-xl border border-amber/25 bg-amber/[0.06] p-2"><div id="needs-daniel-heading" className="hud-label text-amber">Needs Daniel · {fleet.attentionCount}</div>{fleet.truthWarnings.length > 0 && <ul aria-label="Projection reconciliation warnings" className="mt-1 space-y-1">{fleet.truthWarnings.map((warning) => <li key={warning} className="flex gap-2 text-[9px] text-amber"><span aria-hidden="true">!</span><span>{warning}</span></li>)}</ul>}<ul className="mt-1 space-y-1">{attention.map((node) => <li key={node.id}><button type="button" onClick={() => selectJob(node.jobId)} className="flex w-full min-w-0 items-center gap-2 rounded-lg px-1 py-1 text-left hover:bg-white/[0.04]"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber"/><span className="min-w-0 flex-1 truncate text-[10px] text-ice">{agentName(node.agent)} · {node.label}</span><span className="shrink-0 text-[8px] text-amber">{node.decision.kind === "none" ? node.state.replace("_", " ") : node.decision.kind}</span></button></li>)}</ul></section>}
