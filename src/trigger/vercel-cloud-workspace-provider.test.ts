...
const observed: {
  ...
  logs: Log[]; createGate?: Promise<void>; exitCode: number;
} = { commands: [], deletes: [], updates: [], kills: [], logsClosed: 0, files: new Map(), logs: [], exitCode: 0 };

class FakeCommand {
  readonly cmdId = `cmd-${observed.commands.length}`;
  exitCode: number | null = null;
  async wait() { this.exitCode = this.exitCode ?? observed.exitCode; return { exitCode: this.exitCode, durationMs: 1 }; }
...
class FakeSession {
  ...
  async update({ networkPolicy }: { networkPolicy?: unknown }) { this.networkPolicy = networkPolicy; observed.updates.push(networkPolicy); }
}

class FakeSandbox {
  static current = new FakeSession();
  static async list() { return { [Symbol.asyncIterator]: async function* () {} }; }
...
beforeEach(() => {
  ...
  observed.files = new Map(); observed.logs = []; observed.createGate = undefined; observed.exitCode = 0; FakeSandbox.current = new FakeSession();
});
...
describe("VercelCloudWorkspaceProvider", () => {
  it("uses explicit control-plane credentials and a private node22 deny-all session", async () => {
    ...
    expect(workspace).toMatchObject({ providerWorkspaceId: "jarvis-test-name", providerSessionId: "vercel-session-a" });
    ...
    expect(observed.deletes).toEqual(["jarvis-test-name"]);
  });
