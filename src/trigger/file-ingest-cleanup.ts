...
export async function runFileIngestDerivedCleanup(payload: { outboxId: string }) {
  const outboxId = String(payload.outboxId ?? "");
  if (!outboxId) throw new Error("private ingest cleanup outbox identity is missing");
  const claim = await convexMutation("files:claimIngestDerivedCleanup", { outboxId }) as
  ...
  const finished = await convexMutation("files:finishIngestDerivedCleanup", { outboxId });
