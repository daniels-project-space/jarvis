...
type CleanupClaim =
  | { ready: true; assetR2Key: string; claimToken: string; cleanupProtocol: "nonterminal-reaper-v1" }
...
export const creationAssetCleanup = task({
  ...
  run: async (payload: { assetR2Key: string }) => {
    ...
    const canonicalCreation = await convexCall("query", "creations:getByAssetR2Key", { assetR2Key });
    ...
    await privateR2Delete(assetR2Key);
    const finished = await convexCall("mutation", "creationAssetCleanup:finish", {
      assetR2Key,
      claimToken: claim.claimToken,
    }) as CleanupFinish;
