...
describe("Codex app-server protocol", () => {
  ...
  it("prewarms a bounded history-seeded conversation and keeps dynamic tools", async () => {
    ...
    const warmTurn = successor.runTurn({
      ...
      // The predecessor remained available while this process prewarmed and
      // completed one more turn. The successor must append precisely that
      // missing pair before it accepts the next instruction.
      history: [
        ...handoff[0].history,
        { role: "user", text: "during handoff" },
        { role: "assistant", text: "handled by old" },
      ],
    ...
    expect(nextHarness.writes[2]).toEqual({
      id: nextHarness.writes[2].id,
      method: "thread/inject_items",
      params: {
        threadId: "thread-warm",
        items: [
          { type: "message", role: "user", content: [{ type: "input_text", text: "during handoff" }] },
          { type: "message", role: "assistant", content: [{ type: "output_text", text: "handled by old" }] },
        ],
      },
    });
    respond(nextHarness.internals, nextHarness.writes[2], {});
    await vi.waitFor(() => expect(nextHarness.writes).toHaveLength(4));
    expect(nextHarness.writes[3]).toMatchObject({
      method: "turn/start",
      params: {
        threadId: "thread-warm",
    ...
    respond(nextHarness.internals, nextHarness.writes[3], { turn: { id: "turn-warm" } });
    await Promise.resolve();
    ...
    expect(nextHarness.writes.filter((message) => message.method === "thread/inject_items")).toHaveLength(2);
  });

  it("interrupts an over-bound answer instead of retaining unbounded stream text", async () => {
    const server = new CodexAppServer("unused", {} as NodeJS.ProcessEnv, 2_000);
    const { internals, writes } = attach(server);
    const result = server.runTurn({
    ...
    await vi.waitFor(() => expect(writes).toHaveLength(1));
    respond(internals, writes[0], { thread: { id: "thread-bounded" } });
    await vi.waitFor(() => expect(writes).toHaveLength(2));
    respond(internals, writes[1], { turn: { id: "turn-bounded" } });
    await Promise.resolve();
    internals.receive(JSON.stringify({
      method: "item/agentMessage/delta",
      params: {
        ...
        turnId: "turn-bounded",
        itemId: "item-bounded",
        delta: "x".repeat(120_001),
      },
    }));
    await expect(result).rejects.toThrow("exceeded its deterministic bound");
    expect(writes.at(-1)).toMatchObject({
      method: "turn/interrupt",
      params: { threadId: "thread-bounded", turnId: "turn-bounded" },
    });
  });
});
