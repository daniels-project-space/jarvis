...
export function plannerTask(goal: string, route: GoalRoute, acceptanceCriteria: string[], maxBuildSessions: number): string {
  return [
    ...
    `Create only the ${maxBuildSessions === 1 ? "single necessary" : `1-${maxBuildSessions} necessary`} workstream${maxBuildSessions === 1 ? "" : "s"}; never add a ceremonial specialist merely to make a larger crew. Assign Paul to engineering/integration, Atlas to evidence research, Iris to media/visual quality, Maya to travel/calendar work, Chloe to social/content operations, and Sentry to reliability/security review. Give each workstream one structured deliverable, required evidence and explicit guardrails. Do not require a specialist to prove its own post-exit provider-workspace termination; reserve that controller-owned lifecycle check for the final validation audit. Express only real ordering requirements as dependsOn edges; verified upstream outputs become the dependent worker's context. Continue reversible independent work before escalating, and ask Daniel only for the smallest genuinely protected decision. Once every stop condition is proved, stop the crew and leave no polling worker running merely to look busy.`,
