...
export function googleOAuthStatusPresentation(
  ...
  // A stored encrypted account alone is not enough to make a live Gmail
  // request: the Next server also needs its OAuth client and the
  ...
  switch (server) {
    case "needs_setup":
      return {
        label: "needs setup",
        hint: "Google OAuth needs its production client, secret, and token-encryption key before Gmail can run.",
        tone: "attention",
        action: "none",
      };
    case "needs_reconnect":
      return {
        label: "reconnect",
        hint: "Jarvis cannot read the saved Google connection with the current secure settings. Reconnect Google to restore Gmail access.",
        tone: "attention",
        action: "reconnect",
      };
    case "unavailable":
      return {
        label: "check later",
        hint: "Google connection readiness could not be checked. Jarvis will not claim Gmail is ready.",
        tone: "attention",
        action: "none",
      };
    case "checking":
    ...
    case "configured":
      switch (connection) {
        case "connected":
          return {
            label: "connected ✓",
            hint: "Gmail read and draft actions are ready.",
            tone: "ready",
            action: "none",
          };
        case "needs_reconnect":
          return {
            label: "reconnect",
            hint: "Reconnect Google to grant the limited Gmail permissions Jarvis needs.",
            tone: "attention",
            action: "reconnect",
          };
        case "disconnected":
          return {
            label: "connect",
            hint: "Gmail read and draft actions are ready to connect.",
            tone: "neutral",
            action: "connect",
          };
        case "checking":
