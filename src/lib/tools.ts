  ...
  return await withAdminSession(authTokenHash, async () => {
    ...
    case "dispatch_agent": {
      ...
      const agentId = ["paul", "atlas", "iris", "maya", "sentry"].includes(requested)
        ...
        : route.agentId;
      // Keep explicit choices and route-enforced high-risk tiers as quality
      // floors, while allowing routine work to use the shared request-aware
      // policy rather than treating every fast handoff as a generic Terra job.
      const modelPolicy = selectCodexWorkPolicy({
        ...
        requestedModel: args.model ?? (
          route.risk === "high" || route.risk === "consequential"
            ? route.model
            : undefined
        ),
