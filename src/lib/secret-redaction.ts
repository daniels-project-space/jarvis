const SENSITIVE_ENV_NAME = /(?:api_?key|(?:access|refresh|id)_?token|auth(?:orization)?|secret|password|private_?key|project_?id)/i;
...
const ASSIGNMENT_PATTERN = /((?:api[_-]?key|(?:access|refresh|id)[_-]?token|auth(?:orization)?|secret|password|private[_-]?key|project[_-]?id)[\\"']*\s*[:=]\s*[\\"']*)([^\\"'\s,}]{6,})/gi;
