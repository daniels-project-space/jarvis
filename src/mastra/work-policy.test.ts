  ...
  it("does not mistake a no-POST evidence boundary for a POST request", () => {
    ...
    const task = validatorTask({
      ...
      plan: {
        ...
        assumptions: [],
        outcome: {
          objective: "Prove the requested audit result",
          metric: "verified audit findings",
          baseline: "Production state is not yet independently verified",
          target: "Every accepted finding has direct production evidence",
          measurementWindow: "The current audit window",
          evidenceSources: ["Provider and production read-only evidence"],
          stopConditions: ["Every accepted finding is verified"],
        },
        crew: {
          process: "hierarchical",
          manager: "jarvis",
          delegationRules: ["Create only necessary specialist workstreams"],
          humanEscalation: ["Escalate only when protected input is unavoidable"],
