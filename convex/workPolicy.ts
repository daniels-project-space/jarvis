import { requestsConsequentialAction } from "../src/lib/work-safety";
...
export function workApprovalPolicy(input: WorkApprovalInput): { required: boolean; reason?: string } {
  ...
  // `readonly` controls the workspace and delivery path; it is not an approval
  // waiver. This text-level backstop catches a malformed or adversarial route
  // that asks for a real-world action while claiming read-only execution.
  if (requestsConsequentialAction(input.task)) {
