...
describe("private creation media records", () => {
  ...
  it("keeps an existing legacy V1 row idempotent without using that fallback for V2", async () => {
    const t = convexTest(schema, modules);
    const id = await t.run(async (ctx) => await ctx.db.insert("creations", {
      kind: "image",
      title: "Pre-locator V1 row",
      assetR2Key: PRIVATE_KEY,
      assetContentType: "image/png",
      createdAt: Date.now(),
      updatedAt: Date.now(),
    ...
    await expect(t.mutation(api.creations.create, {
      kind: "image",
      title: "A retry must not duplicate the old V1 row",
      assetR2Key: PRIVATE_KEY,
      assetContentType: "image/png",
      workerToken: WORKER,
    })).resolves.toBe(id);
  ...
  it("issues an opaque server deletion ticket for the current cleanup worker and consumes it at finish", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-08-28T12:05:00Z"));
    const t = convexTest(schema, modules);
    const cleanupApi = api as any;
    await t.mutation(cleanupApi.creationAssetCleanup.reserve, {
      assetR2Key: PRIVATE_KEY,
    ...
    await t.mutation(cleanupApi.creationAssetCleanup.abandon, {
      assetR2Key: PRIVATE_KEY,
      writerEpoch: WRITER_EPOCH,
      workerToken: WORKER,
    });
    ...
    expect(claim).toMatchObject({
      ...
      assetStore: "private-r2-v1",
      assetLocator: PRIVATE_KEY,
      deletionTicketId: expect.any(String),
    });
    ...
    await expect(t.run(async (ctx) => await ctx.db.query("creationAssetCleanupTickets").collect())).resolves.toEqual([]);
  });
