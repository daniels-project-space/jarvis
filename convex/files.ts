...
export const authorizeFileTool = query({
  ...
  handler: async (ctx, args) => {
    ...
    const intentRules: Array<[RegExp, RegExp]> = [
      ...
      [/^google_calendar_create$/, /\b(?:google\s*calendar|gcal)\b.{0,48}\b(?:add|create|schedule|put|make|remind)\b|\b(?:add|create|schedule|put|make|remind)\b.{0,48}\b(?:google\s*calendar|gcal)\b/],
      [/^google_calendar_update$/, /\b(?:google\s*calendar|gcal)\b.{0,48}\b(?:change|edit|update|move|reschedule)\b|\b(?:change|edit|update|move|reschedule)\b.{0,48}\b(?:google\s*calendar|gcal)\b/],
      [/^google_calendar_delete$/, /\b(?:google\s*calendar|gcal)\b.{0,48}\b(?:delete|remove|cancel)\b|\b(?:delete|remove|cancel)\b.{0,48}\b(?:google\s*calendar|gcal)\b/],
