...
describe("Goal Mode exact source lineage", () => {
  ...
  it("preserves the sealed branch through adaptively routed bounded work", async () => {
    ...
    const plan = {
      ...
      assumptions: [],
      outcome: {
        objective: "Repair and validate the exact accepted YouTube Studio production source",
        metric: "verified production source-lineage and user-journey checks",
        baseline: "The continuation branch has not yet passed the production validation contract",
        target: "The exact sealed branch passes every accepted production check",
        measurementWindow: "One complete post-deployment validation window",
        evidenceSources: ["Signed source receipts, focused tests, and production provider evidence"],
        stopConditions: ["Every accepted check passes against the exact deployed revision"],
      },
      crew: {
        process: "hierarchical",
        manager: "jarvis",
        delegationRules: ["Create only the evidence and implementation specialists needed for this outcome"],
      ...
      workstreams: [
        {
          ...
          deliverable: {
            kind: "research_brief",
            description: "A source-bound configuration evidence brief",
            requiredEvidence: ["Exact configuration and source-lineage evidence"],
          },
          guardrails: ["Read only; do not mutate the project or providers"],
          mcp: [],
        },
        {
          ...
          deliverable: {
            kind: "code_change",
            description: "A source-bound root-cause repair with deployment evidence",
            requiredEvidence: ["Focused tests, full checks, and exact provider revision evidence"],
          },
          guardrails: ["Do not bypass source, review, deployment, or spend controls"],
          mcp: ["context7"],
        },
      ],
      validation: {
