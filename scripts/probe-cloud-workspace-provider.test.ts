...
describe("Vercel live-provider probe source contract", () => {
  it("uses the same finite owner-scoped page traversal as create-time admission", () => {
    ...
    expect(source).toContain("namePrefix: VERCEL_NAME_PREFIX");
    expect(source).toContain('sortBy: "name"');
    expect(source).toContain('sortOrder: "asc"');
    expect(source).toContain("tags: { owner: \"jarvis\" }");
    expect(source).toContain("limit: VERCEL_HISTORY_PAGE_LIMIT");
    expect(source).toContain("runWithDeadline(30_000, async (signal)");
